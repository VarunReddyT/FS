...
printjson(
	db.stucollection.aggregate([
	    ...
	    {$unwind : "$exams.scores"},
	    {$group : {_id:"$student_id",name:{$first:"$name"},avgPythonScore : {$avg:"$exams.scores.score"}}},
	    {$sort : {avgPythonScore : -1}},
	    {$limit : 1},
	    {$project : {student_id:"$_id",name:"$name",avgPythonScore:{$round:["$avgPythonScore",2]},_id:0}}
	])
)
