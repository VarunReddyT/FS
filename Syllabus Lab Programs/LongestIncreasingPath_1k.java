...
public class LongestIncreasingPath_1k {
    ...
    private int dfs(int[][] matrix, int[][] dp, int row, int col){
        ...
        for(int c = 0;c<4;c++){
            ...
            if(i >= 0 && i < m && j >= 0 && j < n && matrix[i][j] > matrix[row][col]){
                currentLongest = Math.max(currentLongest, 1 + dfs(matrix, dp, i, j));
            }
        }
        dp[row][col] = currentLongest;
        return dp[row][col];
    }
    public static void main(String[] args) {
        ...
        System.out.println(new LongestIncreasingPath_1k().longestIncreasingPath(matrix));
        sc.close();
    }
}
