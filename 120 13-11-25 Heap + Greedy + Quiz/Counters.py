...
def assign_orders(s, t, capacities, orders):
    ...
    for i in range(s):
        heapq.heappush(available_counters, (capacities[i], i))

    current_time = 0
    for k in range(t):
        current_time = max(current_time, k)

        while busy_counters and busy_counters[0][0] <= current_time:
            free_time, cap, idx = heapq.heappop(busy_counters)
            heapq.heappush(available_counters, (cap, idx))
        ...
        cap, idx = heapq.heappop(available_counters)
        result[k] = idx
        finish_time = current_time + orders[k]
        heapq.heappush(busy_counters, (finish_time, cap, idx))

    return result

assigned_counters = assign_orders(s, t, capacities, orders)
print(' '.join(map(str, assigned_counters)))