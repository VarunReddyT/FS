...
public class getRoomSizes{
    public static int getSize(int n, int maxFreq, int maxArea, int[] arr){
        int low = 0;
        int high = 0;
        ...
        while(high<n){
            ...
            area += arr[high];
            while((map.get(arr[high]) > maxFreq || area>maxArea) && low<=high){
    ...
    public static void main(String[] args){
        ...
        System.out.println(getSize(n,maxFreq,maxArea,arr));
        sc.close();
    }
}