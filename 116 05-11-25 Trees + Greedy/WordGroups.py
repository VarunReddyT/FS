...
def getPairs(words):
    ...
    for i in range(n):
        ...
        for j in range(n):
            word1 = words[i]
            word2 = words[j]
            if i == j or visited[j] or len(word1) != len(word2):
                continue
            
            flag = False
            for k in range(1, len(word1)):
                if (ord(word1[k]) - ord(word1[k-1]) + 26)%26 != (ord(word2[k]) - ord(word2[k-1]) + 26)%26:
                    flag = True
                    break
